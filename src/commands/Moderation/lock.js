...
module.exports = {
    ...
    async slashExecute(interaction, client) {
        ...
        try {
            ...
            if (isLocked) {
                const display = new TextDisplayBuilder().setContent(`${emoji.warn} ${channel} is already locked.`);
                return interaction.reply({ components: [new ContainerBuilder().addTextDisplayComponents(display)], flags: MessageFlags.IsComponentsV2 });
            }
            ...
            const display = new TextDisplayBuilder().setContent(`${emoji.check} ${channel} has been locked.`);
            return interaction.reply({ components: [new ContainerBuilder().addTextDisplayComponents(display)], flags: MessageFlags.IsComponentsV2 });
        } catch (error) {
            const display = new TextDisplayBuilder().setContent(`${emoji.warn} Failed to lock channel: ${error.message}`);
            return interaction.reply({ components: [new ContainerBuilder().addTextDisplayComponents(display)], flags: MessageFlags.IsComponentsV2 });
        }
    },
